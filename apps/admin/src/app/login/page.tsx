...
export default function LoginPage() {
  ...
  const { firebaseUser, isAdmin, isMarketing, loading } = useAuth();
  ...
  useEffect(() => {
    if (!loading && firebaseUser) {
      ...
      else if (isMarketing) router.replace("/marketing");
    }
  }, [loading, firebaseUser, isAdmin, isMarketing, router]);

  // Route by role after any successful sign-in; deny accounts with neither role.
  async function routeByRole(uid: string): Promise<void> {
    ...
    await signOut(auth());
    setError("Access denied. This account doesn't have dashboard access.");
    setSigningIn(false);
  }
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (code === "auth/operation-not-allowed") {
        setError("Email sign-in isn't enabled for this project yet.");
      } else {
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-8 shadow-xl backdrop-blur-sm">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-3 bg-[var(--card)] text-[var(--muted)]">or sign in with email</span>
            </div>
          </div>

          {/* Email + password (marketing staff and admins) */}
          <form onSubmit={handleEmailSignIn} className="space-y-3">
            ...
            <Input
              ...
              className="w-full"
            />
            <Button type="submit" loading={signingIn} size="lg" className="w-full">
              Sign in
            </Button>
          </form>
